...
import { searchSaint, checkInSaint } from '../api/saints'
import type { Saint } from '../api/types'

type CheckInState = 'form' | 'confirm' | 'success'
...
export default function EventCheckIn() {
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    try {
      const found = await searchSaint(firstName.trim(), lastName.trim())
      if (found) {
        setSaint(found)
        setState('confirm')
      } else {
  ...
  const handleConfirmCheckIn = async () => {
    ...
    try {
      ...
      navigate(`/welcome?first=${encodeURIComponent(saint.first_name)}&returning=true&redirect=/event`, { replace: true })
    } catch {
  ...
  if (state === 'confirm' && saint) {
    const fullName = `${saint.first_name} ${saint.last_name}`.trim()
    const initials = `${saint.first_name[0] ?? ''}${saint.last_name[0] ?? ''}`
    return (
        ...
        <main className="flex-1 -mt-4 rounded-t-[2rem] bg-surface-container-lowest px-6 pt-10 pb-12 animate-fade-up">
          <div className="max-w-sm mx-auto flex flex-col gap-6">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-on-surface-variant">
                  We found your registration. Confirm your details below.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={resetForm} className="btn-tertiary w-full text-sm">
                I'm not {fullName}
              </button>
            </div>
          </div>
